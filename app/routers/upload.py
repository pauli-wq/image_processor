...
async def get_status(task_id: str):
    clean_task_id = task_id.strip('"').strip("'")
    res = celery_app.AsyncResult(clean_task_id)
    if res.status == "PENDING" and not res.result:
        return TaskResult(task_id=clean_task_id, status="PENDING")
    return TaskResult(task_id=clean_task_id, status=res.status)
